...
// DecodeHTTPGetOneZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded getone request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPGetOneZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.GetOneUserRequest
	...
	if IdGetOneStrArr, ok := queryParams["id"]; ok {
		IdGetOneStr := IdGetOneStrArr[0]
		IdGetOne, err := strconv.ParseInt(IdGetOneStr, 10, 32)
		if err != nil {
			return nil, errors.Wrap(err, fmt.Sprintf("Error while extracting IdGetOne from query, queryParams: %v", queryParams))
		}
		req.Id = int32(IdGetOne)
	}
